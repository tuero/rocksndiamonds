/**
 * @file: engine_element_property.cpp
 *
...
bool isTemporaryElement(const enginetype::GridCell &cell) {
    return gridinfo::inBounds(cell) && Feld[cell.x][cell.y] >= NUM_DRAWABLE_ELEMENTS;
}
...
bool isMoving(const enginetype::GridCell cell) {
    return gridinfo::inBounds(cell) && IS_MOVING(cell.x, cell.y);
}
...
bool _isAccessibleFromDirection(enginetype::GridCell cellTo, Action directionToMove) {
    ...
    return (element_info[element].access_direction & (actioninfo::getOppositeDirection(directionToMove)));
}
...
bool isDigable(const enginetype::GridCell cellFrom, Action directionToMove) {
    enginetype::GridCell cellTo = gridaction::getCellFromAction(cellFrom, directionToMove);
    return gridinfo::inBounds(cellTo) && _isDigable(cellTo);
}
...
bool isWall(const enginetype::GridCell cellFrom, Action directionToMove) {
    enginetype::GridCell cellTo = gridaction::getCellFromAction(cellFrom, directionToMove);
    return gridinfo::inBounds(cellTo) && _isWall(cellTo);
}
...
bool isRock(const enginetype::GridCell cellFrom, Action directionToMove) {
    enginetype::GridCell cellTo = gridaction::getCellFromAction(cellFrom, directionToMove);
    return gridinfo::inBounds(cellTo) && _isRock(cellTo);
}
 ...
 * For internal use, assumes bounds are validated elsewhere.
 */
bool _isPushable(const enginetype::GridCell cellTo) {
    int element = Feld[cellTo.x][cellTo.y];
    return IS_PUSHABLE(element);
}
 ...
 * Checks if the player can push an element in the direction it wants to move.
 */
bool isPushable(const enginetype::GridCell cellFrom, Action directionToMove) {
    enginetype::GridCell cellTo = gridaction::getCellFromAction(cellFrom, directionToMove);
    return gridinfo::inBounds(cellTo) && _isPushable(cellTo);
}
...
bool isCollectable(const enginetype::GridCell cellFrom, Action directionToMove) {
    enginetype::GridCell cellTo = gridaction::getCellFromAction(cellFrom, directionToMove);
    return gridinfo::inBounds(cellTo) && _isCollectable(cellTo);
}
...
bool isPassable(const enginetype::GridCell cellFrom, Action directionToMove) {
    ...
    enginetype::GridCell cellPassed = gridaction::getCellFromAction(cellFrom, directionToMove);
    return gridinfo::inBounds(cellPassed) && _isPassable(cellPassed, directionToMove);
}
